...
class dadagpDatasetStats(Dataset):
    ...
    def _generate_samples(self):
        ...
        for filename, song_df in self.songs:
            ...
            for t in range(num_rows):  # Stop at second-last row
                chord_history = song_df.iloc[t]["chordname"]
                position = song_df.iloc[t]["position"]

                # **Skip t if any element in history or target is None**
                if (not isinstance(chord_history, str) and math.isnan(chord_history)) or \
                    (not isinstance(position, str) and math.isnan(position)):
                    # print(f"first: '{chord_history}' '{position}")
                    continue
                if any(any(solfege in chord for solfege in dont_include) for chord in [chord_history]):
                    print(f"second: '{chord_history}' '{position}")
                    continue  # Skip this progression
                if any(any(solfege in chord.lower() for solfege in dont_include) for chord in [chord_history]):
                    print(f"third: '{chord_history}' '{position}")
                    continue  # Skip this progression

                if any( chord.endswith('sus') for chord in [chord_history]):
                    print(f"fourth: '{chord_history}' '{position}")
                    continue  # Skip this progression

                if any('\\' in [i for i,j in Counter(chord).items() if j>1] for chord in [chord_history]):
                    print(f"fifth: '{chord_history}' '{position}")
                    continue
                if any('/' in [i for i,j in Counter(chord).items() if j>1] for chord in [chord_history]):
                    print(f"sixth: '{chord_history}' '{position}")
                    continue

                try:
                    self.encode_chord(chord_history)
                except Exception as e:
                    print(chord_history, e)
                    continue

                # print(chord_history, position)

                output_records.append({
                    "filename": filename,  # Keep track of the song
                    "measure": t,
                    "chords": chord_history,  # List of previous chords
                    "position": position,  # List of previous positions
                })
            # print(song_df)
        ...
        df.to_csv("dadagp_voiced_chord_sequence.csv", index=False)
...
dataset = dadagpDatasetStats('.', history_length=7)
